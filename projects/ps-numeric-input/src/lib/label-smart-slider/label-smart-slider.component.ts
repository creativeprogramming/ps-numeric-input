...
export const MAT_SLIDER_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => LabelSmartSlider),
  multi: true
};
 ...
 */
@Component({
  selector: 'label-smart-slider',
  exportAs: 'labelSmartSlider',
  ...
  templateUrl: './label-smart-slider.component.html',
  styleUrls: ['./label-smart-slider.component.scss'],
  inputs: ['disabled', 'color', 'tabIndex'],
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class LabelSmartSlider extends _MatSliderMixinBase
  implements ControlValueAccessor, OnDestroy, CanDisable, CanColor, AfterViewInit, HasTabIndex {
  @Input()
  preventSliderClick: boolean;
  intervalTimer;
  ...
  @Output() readonly sliderInput: EventEmitter<number> = new EventEmitter<number>();
  ...
  private _pointerDown = (event: TouchEvent | MouseEvent) => {
    ...
    this._ngZone.run(() => {
      ...
      if (oldValue != this.value && !this.preventSliderClick) {
        this._emitInputEvent();
        this._emitChangeEvent();
      }
    });
  ...
  private _updateValueFromPosition(pos: { x: number, y: number }) {
    ...
    let percent = this._clamp((posComponent - (offset - size / 2)) / size);

    if (this._shouldInvertMouseCoords()) {
      percent = 1 - percent;
    }

    const absPerc = Math.abs(percent - 0.5);

    intervalMs = intervalMs / absPerc;
    intervalMs = intervalMs == 0  ? 60 : intervalMs;
    intervalMs = intervalMs == 1  ? 1 : intervalMs*4;

    intervalMs = Math.round((intervalMs * 1000) / (this.max - this.min));

    intervalMs = intervalMs > 500 ? 500: intervalMs;
    ...
    this.intervalTimer = setInterval(() => {
      ...
      elapsed = intervalMs == 1 ? elapsed * this.step : this.step;
      ...
      const exactValue = percent < 0.5 ? this.value - elapsed : this.value + elapsed;//this._calculateValue(percent);
  ...
  private _emitInputEvent() {
    ...
    this.onSliderInput(this._createChangeEvent());
  }
